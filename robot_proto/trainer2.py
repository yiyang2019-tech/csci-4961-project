...
def main():
    ...
    global_step= tf.Variable(start_step,trainable=False,name='global_step3')
    datasets= DataReader(batch_size= 5, hw_root='datasets/train')
    ...
    decay_steps=int(datasets.len()/BATCH_SIZE*3)
    lr_tf=tf.train.exponential_decay(0.01,global_step,decay_steps,0.94,staircase=True)
    opt =tf.train.AdamOptimizer(lr_tf,name="Adam_15")
    print(1)
